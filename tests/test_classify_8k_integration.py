...
Event Type: Financial Event, Relevant: true"""
        ...
        self.real_sec_url = (
            "https://www.sec.gov/Archives/edgar/data/320193/000119312521328151/d259993d8k.htm"
        )
    ...
    def test_real_data_integration_board_appointment(self):
        ...
        try:
            ...
            result = classify_8k_filing(
                input_source=self.real_sec_url, strategy=PromptStrategy.DETAILED, verbose=True
            )
    ...
    def test_classify_8k_filing_success_mocked(self, mock_classify):
        ...
        result = classify_8k_filing(
            input_source=self.sample_8k_path, strategy=PromptStrategy.DETAILED, verbose=False
        )
    ...
    def test_classify_8k_filing_different_strategies(self, mock_classify):
        ...
        for strategy in strategies:
            with self.subTest(strategy=strategy):
                result = classify_8k_filing(
                    input_source=self.sample_8k_path, strategy=strategy, verbose=False
                )
    ...
    def test_classify_8k_filing_file_not_found(self):
        ...
        result = classify_8k_filing(
            input_source="non_existent_file.html", strategy=PromptStrategy.DETAILED, verbose=False
        )
    ...
    def test_classify_8k_filing_url_download_mocked(
        self, mock_load_config, mock_llm_client, mock_requests
    ):
        ...
        result = classify_8k_filing(
            input_source=test_url, strategy=PromptStrategy.DETAILED, verbose=False
        )
    ...
    def test_verbose_logging_output(self, mock_load_config, mock_llm_client):
        ...
        try:
            result = classify_8k_filing(
                input_source=self.sample_8k_path, strategy=PromptStrategy.DETAILED, verbose=True
            )
    ...
    def test_sample_fixture_contains_expected_content(self):
        ...
        self.assertTrue(
            os.path.exists(self.sample_8k_path),
        ...
        for expected in expected_content:
            self.assertIn(
                expected, content, f"Expected content '{expected}' not found in sample fixture"
            )
