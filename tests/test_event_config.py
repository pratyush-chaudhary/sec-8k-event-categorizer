...
class TestEventConfig(unittest.TestCase):
    ...
    def test_default_config_structure_integrity(self):
        ...
        for event_type, event_config in config.items():
            ...
            self.assertTrue(
                len(event_config.description) > 0, f"{event_type} should have a description"
            )
            ...
            if event_config.relevant and event_type != "Other":
                self.assertTrue(
                    len(event_config.keywords) > 0, f"{event_type} should have keywords if relevant"
                )
...
class TestClassificationPrompts(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        self.sample_text = (
            "Apple Inc. announced the acquisition of XYZ Corp for $1.2 billion in cash and stock."
        )
        self.event_types = [
            "Acquisition",
            "Customer Event",
    ...
    def test_basic_classification_prompt(self):
        ...
        prompt = ClassificationPrompts.basic_classification_prompt(
            self.sample_text, self.event_types
        )
    ...
    def test_detailed_classification_prompt(self):
        ...
        prompt = ClassificationPrompts.detailed_classification_prompt(
            self.sample_text, self.event_configs
        )
    ...
    def test_few_shot_prompt_with_custom_examples(self):
        ...
        prompt = ClassificationPrompts.few_shot_prompt(
            self.sample_text, self.event_types, custom_examples
        )
    ...
    def test_validation_prompt(self):
        ...
        prompt = ClassificationPrompts.validation_prompt(
            self.sample_text, classification, self.event_types
        )
    ...
    def test_convenience_functions(self):
        ...
        expected_basic = ClassificationPrompts.basic_classification_prompt(
            self.sample_text, self.event_types
        )
        ...
        expected_detailed = ClassificationPrompts.detailed_classification_prompt(
            self.sample_text, self.event_configs
        )
        ...
        expected_cot = ClassificationPrompts.chain_of_thought_prompt(
            self.sample_text, self.event_types
        )
        ...
        expected_few_shot = ClassificationPrompts.few_shot_prompt(
            self.sample_text, self.event_types
        )
    ...
    def test_prompt_length_and_structure(self):
        ...
        prompts = {
            "basic": ClassificationPrompts.basic_classification_prompt(
                self.sample_text, self.event_types
            ),
            "detailed": ClassificationPrompts.detailed_classification_prompt(
                self.sample_text, self.event_configs
            ),
            "cot": ClassificationPrompts.chain_of_thought_prompt(
                self.sample_text, self.event_types
            ),
        ...
        for prompt_type, prompt in prompts.items():
            ...
            self.assertTrue(
                ...
                or "Classification" in prompt[-50:]
                or "Analysis" in prompt[-50:],
